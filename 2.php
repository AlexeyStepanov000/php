...
function Draw($s, $c2, $height, $weight) {
	for ($k = 1; $k < 10; $k++) {
		if (@$_SESSION['value' . $k] == 1) {
			imageline ($s, (($weight - 100) / 3) * (($k - 1) % 3), (($height - 100) / 3) * intdiv(($k - 1), 3), (($weight - 100) / 3) * (($k - 1) % 3 + 1), (($height - 100) / 3) * (intdiv(($k - 1), 3) + 1), $c2);
			imageline ($s, (($weight - 100) / 3) * (($k - 1) % 3), (($height - 100) / 3) * (intdiv(($k - 1), 3) + 1), (($weight - 100) / 3) * (($k - 1) % 3 + 1), (($height - 100) / 3) * intdiv(($k - 1), 3), $c2);
		} else if (@$_SESSION['value' . $k] == 2) {
			imageellipse($s, (($weight - 100) / 6) * ((($k - 1) % 3 + 1) + 3 - (3 - (($k - 1) % 3))), (($weight - 100) / 6) * (1 + intdiv($k - 1, 3) * 2), ($weight - 100) / 3, ($weight - 100) / 3, $c2);
		}	
	}
}

function Check($s, $c2, $c3, $height, $weight, $i, $j, $k, $SESSION, $Link, $Query) {
	if (@$_SESSION['value' . $i] == @$_SESSION['value' . $j] and @$_SESSION['value' . $k] == @$_SESSION['value' . $j] and (@$_SESSION['value' . $j] == 1 or @$_SESSION['value' . $j] == 2)) {
...
session_start();
$Query = "insert into log (id, description)
	values(0, reset)";
	mysqli_query($Link, $Query);
